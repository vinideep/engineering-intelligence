...
> Skill files are in `.agents/skills/<name>/` (SKILL-BRIEF.md and SKILL.md).

| Command | Primary Skills — load first | Optional Skills — load if needed |
|---|---|---|
| `engineering-intelligence` | `engineering-intelligence-skill`, `aidlc-lifecycle-engine`, `impact-analysis-engine`, `context-budget-optimizer` | `user-intelligence-engine`, `change-detection-engine`, `incremental-sync-engine`, `change-history-engine`, `environmental-backpressure-engine`, `testing-intelligence-engine`, `question-file-engine` |
...
| `sync-engineering-intelligence` | `change-detection-engine`, `incremental-sync-engine` | `knowledge-base-validator`, `graph-engine` |
