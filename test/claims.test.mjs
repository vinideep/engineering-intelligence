...
test("addClaim computes hashes and persists; verify passes while source is unchanged", async () => {
  const root = await tmp();
  try {
    await write(root, "src/pay.ts", "export function pay() {\n  return charge();\n}\n");
    const claim = await addClaim(root, { statement: "pay delegates to charge", evidence: [{ path: "src/pay.ts", lines: [1, 3] }] });
...
test("editing a cited line flips the claim stale", async () => {
  ...
  try {
    await write(root, "src/pay.ts", "export function pay() {\n  return charge();\n}\n");
    await addClaim(root, { statement: "pay delegates to charge", evidence: [{ path: "src/pay.ts", lines: [1, 3] }] });
    await write(root, "src/pay.ts", "export function pay() {\n  return refund();\n}\n");
    const report = await verifyClaims(root);
...
test("deleting the cited file flips the claim missing", async () => {
  ...
  try {
    await write(root, "src/pay.ts", "export function pay() {}\n");
    await addClaim(root, { statement: "pay exists", evidence: [{ path: "src/pay.ts" }] });
...
test("changes OUTSIDE the cited line range do not flip the claim stale", async () => {
  ...
  try {
    ...
    const report = await verifyClaims(root);
    assert.equal(report.verified, 1, "line-scoped evidence ignores unrelated edits");
  } finally { await rm(root, { recursive: true, force: true }); }
});
...
test("trailing-whitespace-only edits do not flip the claim stale", async () => {
  ...
  try {
    ...
    await addClaim(root, { statement: "x and y", evidence: [{ path: "src/a.ts", lines: [1, 2] }] });
    await write(root, "src/a.ts", "const x = 1;   \nconst y = 2;\t\n");
    const report = await verifyClaims(root);
    assert.equal(report.verified, 1, "normalization ignores trailing whitespace");
  } finally { await rm(root, { recursive: true, force: true }); }
});
